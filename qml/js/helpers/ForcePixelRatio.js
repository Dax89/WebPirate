...
var __wp_forcepixelratio__ = {
    adjust: function() {
        ...
        if(screen.width <= 540) /* Jolla devicePixelRatio: 1.5 */
            viewport.setAttribute('content', 'width=device-width/1.5, initial-scale='+(1.5));
        else if(screen.width > 540 && screen.width <= 768) /* Nexus 4 devicePixelRatio: 2.0 */
            viewport.setAttribute('content', 'width=device-width/2.0, initial-scale='+(2.0));
        else if (screen.width > 768) /* Nexus 5 devicePixelRatio: 3.0 */
            viewport.setAttribute('content', 'width=device-width/3.0, initial-scale='+(3.0));
    }
}
